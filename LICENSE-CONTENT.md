# License for the educational content

**Creative Commons Attribution 4.0 International (CC BY 4.0)**

© 2026 Youth of Change

## What this covers

The lesson plans and all lesson text (`js/lessons.js`), the slide decks
(`slides/`), and the artwork and illustrations (`assets/`).

The website code — the HTML, CSS and JavaScript in `index.html`, `css/` and
`js/` — is covered separately by the MIT license in [LICENSE](LICENSE).

## What you may do

You are free to:

- **Share** — copy and redistribute the material in any medium or format
- **Adapt** — remix, transform and build upon the material

for any purpose, including commercially.

These freedoms cannot be revoked as long as you follow the license terms.

## What we ask in return

**Attribution.** You must give appropriate credit, link to the license, and
indicate if you made changes. You may do so in any reasonable manner, but not
in any way that suggests Youth of Change endorses you or your use.

A suggested credit line:

> "AI Literacy Toolkit" by Youth of Change, licensed under CC BY 4.0.
> https://github.com/yanj428/ai-literacy-toolkit

If you translate or adapt a lesson, please say so — for example, "adapted from
the AI Literacy Toolkit by Youth of Change (CC BY 4.0)".

**No additional restrictions.** You may not apply legal terms or technological
measures that legally restrict others from doing anything the license permits.

## Notices

You do not have to comply with the license for elements of the material in the
public domain, or where an exception or limitation to copyright applies.

No warranties are given. The license may not give you all of the permissions
necessary for your intended use — for example, other rights such as publicity
or privacy rights may limit how you use the material.

## Full license text

This is a human-readable summary of, and not a substitute for, the license.
The authoritative legal text is at:

https://creativecommons.org/licenses/by/4.0/legalcode

The summary is at:

https://creativecommons.org/licenses/by/4.0/

## Third-party material

Some lessons link to resources published by others — for example the
"AI for Oceans" activity at ailiteracyframework.org. Those resources are not
covered by this license and remain under their own terms.
